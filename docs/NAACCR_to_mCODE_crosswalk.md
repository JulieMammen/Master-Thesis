...
The conceptual NAACCR-style field names used in this crosswalk are mapped to the actual columns in `breast_registry_synth_1000.csv` as follows:

| Crosswalk / NAACCR-style name          | Actual CSV column       | Notes                                      |
|----------------------------------------|-------------------------|--------------------------------------------|
| `patientIdNumber`                      | `patient_id`            | Primary patient identifier                 |
| `sexAssignedAtBirth`                   | `sex`                   | Text values (Female / Male)               |
| `race1`                                | `race`                  | Free-text race description                 |
| `dateOfDiagnosis`                      | `date_diagnosed`        | Already in YYYY-MM-DD format               |
| `primarySite`                          | `icd10_code`            | ICD-10-CM (C50.*)                          |
| `summaryStage2018` / stage             | `stage_group`           | Roman numeral stage group                  |
| `tumorSizeSummary`                     | `tumor_size_cm`         | Stored in cm; converted to mm in generator |
| `estrogenReceptorSummary`              | `er_status`              | Positive / Negative / Unknown              |
| `progesteroneRecepSummary`             | `pr_status`              | Positive / Negative / Unknown              |
| `her2OverallSummary`                   | `her2_status`            | Positive / Negative / Unknown              |
| `oncotypeDxRecurrenceScoreInvasiv`     | `oncotype_dx_score`     | Numeric score when present                 |
| `ageAtDiagnosis`                       | `age`                   | Already calculated                         |

Fields that exist only in the full 25-field NAACCR extract (Gleason, PSA, Breslow, FIGO, Ki-67, etc.) are documented in this crosswalk for completeness but are not present in the current breast-only CSV and are therefore not generated by the current script.
